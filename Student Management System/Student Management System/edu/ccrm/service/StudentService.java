...
public class StudentService {
    ...
    public void enrollStudent(String studentId, String courseCode) 
        ...
        boolean alreadyEnrolled = studentEnrollments.stream()
                .anyMatch(enrollment -> enrollment.getCourseCode().equals(courseCode) && enrollment.isActive());
        ...
        int currentCredits = studentEnrollments.stream()
                ...
                .mapToInt(activeEnrollment -> courses.get(activeEnrollment.getCourseCode()).getCredits())
                .sum();
    ...
    public void unenrollStudent(String studentId, String courseCode) {
        ...
        if (studentEnrollments != null) {
            studentEnrollments.stream()
                    .filter(enrollment -> enrollment.getCourseCode().equals(courseCode) && enrollment.isActive())
                    .findFirst()
                    .ifPresent(foundEnrollment -> {
                        foundEnrollment.setActive(false);
    ...
    public void recordGrade(String studentId, String courseCode, double marks) {
        ...
        if (studentEnrollments != null) {
            studentEnrollments.stream()
                    .filter(enrollment -> enrollment.getCourseCode().equals(courseCode) && enrollment.isActive())
                    .findFirst()
                    .ifPresent(activeEnrollment -> activeEnrollment.recordGrade(marks));
    ...
    private void updateStudentGPA(String studentId) {
        ...
        if (studentEnrollments != null) {
            double totalPoints = studentEnrollments.stream()
                    ...
                    .mapToDouble(gradedEnrollment -> {
                        Course course = courses.get(gradedEnrollment.getCourseCode());
                        return gradedEnrollment.getGrade().getPoints() * course.getCredits();
                    })
                    .sum();
            ...
            int totalCredits = studentEnrollments.stream()
                    .filter(Enrollment::isActive)
                    .filter(enrollment -> enrollment.getGrade() != null)
                    .mapToInt(gradedEnrollment -> courses.get(gradedEnrollment.getCourseCode()).getCredits())
                    .sum();
